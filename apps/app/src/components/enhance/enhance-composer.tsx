...
import { createEnhanceGenerationRequestSchema } from "@workspace/shared/api/enhance/schemas"
import type {
...
type EnhanceComposerProps = {
  ...
  onSubmit: (values: CreateEnhanceGenerationRequest) => Promise<unknown>
}
...
export function EnhanceComposer({
  ...
  const form = useForm<CreateEnhanceGenerationRequest>({
    resolver: zodResolver(createEnhanceGenerationRequestSchema),
  ...
  async function submit(values: CreateEnhanceGenerationRequest) {
    await onSubmit({
      ...values,
