...
import { aspectRatioToCss } from "@workspace/shared/api/image/models"
...
export function GenerationImageGrid({ generation }: GenerationImageGridProps) {
  const aspectRatioCss = aspectRatioToCss(generation.aspectRatio)
  const [lightbox, setLightbox] = useState<LightboxState | null>(null)
  const count = generation.images.length
  const [width, height] = generation.aspectRatio.split(":").map(Number)
  const aspect = width && height ? width / height : 1
  const filename = `${generation.prompt.slice(0, 40)}.png`
