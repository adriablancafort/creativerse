...
import { createVideoTurnRequestSchema } from "@workspace/shared/api/create-video/schemas"
import type {
...
import { ModelPicker } from "@/components/create-video/model-picker"
...
type CreateVideoComposerProps = {
  pending?: boolean
  isSubmitting: boolean
  initialStartFrameUrl?: string | null
  onSubmit: (values: CreateVideoTurnRequest) => Promise<unknown>
}
...
export function CreateVideoComposer({
  ...
  async function uploadFrame(kind: "start" | "end", file: File) {
    ...
    try {
      ...
      const uploaded = await api.upload<CreateVideoUploadResponse>(
        "/api/create-video/uploads",
        body
      )
      form.setValue(
