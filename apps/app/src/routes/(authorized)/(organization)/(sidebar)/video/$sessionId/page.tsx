...
import { GenerationTurn } from "@/components/video/generation-turn"
import { VideoComposer } from "@/components/video/video-composer"
import { api } from "@/lib/api"

function hasPendingGenerations(generations: VideoGeneration[]) {
  return generations.some((generation) => generation.status === "pending")
}

function videoSessionQueryOptions(sessionId: string) {
  return queryOptions({
    queryKey: ["video-session", sessionId],
    queryFn: () =>
      api.get<VideoSessionResponse>(`/api/video/sessions/${sessionId}`),
...
function Composer({
  ...
  const addGenerationMutation = useMutation({
    mutationFn: (values: CreateVideoGenerationRequest) =>
      api.post<VideoGenerationResponse>(
        `/api/video/sessions/${sessionId}/generations`,
        { body: values }
      ),
    onSuccess: () => {
...
function Page() {
  ...
  const { data: session } = useSuspenseQuery(
    videoSessionQueryOptions(sessionId)
  )
